...
void *stack_alloc(MemoryBlock **const memory_block, const size_t allocation_size, const size_t alignment) {
	ASSERT_CRASH(memory_block && (*memory_block), "Cannot allocate memory from a null pointer");
	ASSERT_CRASH((*memory_block)->memory, "Cannot allocate memory from null pointer to memory");
	...
	ASSERT_CRASH(0, "Stack Allocator failed to allocate memory");
}

bool stack_alloc_verify(MemoryBlock *const memory_block, const size_t allocation_size, const size_t alignment) {
	ASSERT_CRASH(memory_block, "Cannot verify available memory for NULL pointer");
